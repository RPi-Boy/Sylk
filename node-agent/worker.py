...
def reap_zombies():
    ...
    try:
        containers = docker_client.containers.list(
            all=True, filters={"label": "sylk=true"}
        )
        for c in containers:
...
def start_warm_container(lang="node"):
    ...
    try:
        ...
        print(
            f"Failed to start warm {lang} container: {e}. Check if image {image} exists."
        )
...
if __name__ == "__main__":
    ...
    while True:
        if not watchdog.is_busy():
            ...
            print(
                f"System busy (CPU: {watchdog.get_cpu_average():.1f}%), paused polling..."
            )
            time.sleep(5)
        time.sleep(1)
